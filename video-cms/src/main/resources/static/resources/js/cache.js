...
layui.use(['form','jquery',"layer"],function() {
    ...
    if(!/http(s*):\/\//.test(location.href)){
        ...
        if(window.sessionStorage.getItem("lockcms") != "true" && window.sessionStorage.getItem("showNews") != "true"){
            showNews();
        }
    }
    ...
    //公告层
    function showNews(){
    	$.post("/system/news/loadOneNews.html",function(res){
    		var res=$.parseJSON(res);
    		var news=res.data[0];
    		var html='<div id="desk_viewNewsDiv" style="padding: 10px;">'+
			            '<h2 id="view_title" align="center">'+news.title+'</h2>'+
			    		'<hr>'+
			    		'<div style="text-align: center;">'+
			    			'发布人:<span id="view_publisher">'+news.publisher+'</span><span style="display: inline-block;width: 40px" ></span>'+
			    			'发布时间:<span id="view_createtime">'+news.createTime+'</span>'+
			    		'</div>'+
			    		'<hr>'+
			    		'<div id="view_content">'+news.content+'</div>'+
			    	'</div>';
    		if(res.code==0){
    			layer.open({
    	            type: 1,
    ...
    function tipsShow(){
        window.sessionStorage.setItem("showNews","true");
        if($(window).width() > 432){  //如果页面宽度不足以显示顶部“系统公告”按钮，则不提示
            layer.tips('系统公告躲在了这里', '#administratorInfo', {
                tips: 3,
                time : 1000
            });
        }

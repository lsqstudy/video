...
import com.lsqstudy.dao.ITVPlayDao;
...
public class TVPlayServiceImpl implements ITVPlayService {
    ...
    @Override
    public Page<TVPlay> findAll(VideoVo videoVo) {
        ...
        Example<TVPlay> example = Example.of(videoVo.getTVPlay(), matcher);
    ...
    @Override
    public Page<TVPlay> findByFilter(VideoVo videoVo) {
        ...
        List<TVPlay> tvPlayList = mongoTemplate.find(query, TVPlay.class);
        ...
        // 查询记录总数
        Long totalCount = mongoTemplate.count(basicQuery, TVPlay.class);
        totalCount = totalCount+videoVo.getLimit();//处理少了一页的问题
        //设置分页条件
        Pageable pageable = PageRequest.of(page, limit, sort);
        Page<TVPlay> resultPage = new PageImpl<>(tvPlayList, pageable, totalCount);
    ...
    public TVPlay findTVPlayById(String id) {
        TVPlay tvPlay = mongoTemplate.findById(id, TVPlay.class);
        return tvPlay;
    }
    ...
    public Map<String,Object> search(VideoVo videoVo) {
        ...
        //设置分页条件
        Pageable pageable = PageRequest.of(videoVo.getPage(), videoVo.getLimit(), sort);
        Page<TVPlay> titlePage = tVPlayDao.findByTitleLike(videoVo.getContent(), pageable);
        ;
        Page<TVPlay> typePage = tVPlayDao.findByTypeLike(videoVo.getContent(), pageable);
        Page<TVPlay> actorPage = tVPlayDao.findByActorLike(videoVo.getContent(), pageable);
    ...
    @Override
    public Page<TVPlay> finBannerList(VideoVo videoVo) {
        //设置排序条件
        Sort sort = Sort.by(Sort.Order.desc("updateTime"));
        //设置分页条件
        Pageable pageable = PageRequest.of(videoVo.getPage(), videoVo.getLimit(), sort);
        Page<TVPlay> p = tVPlayDao.findByIsBanner(Constast.IS_BANNER, pageable);
